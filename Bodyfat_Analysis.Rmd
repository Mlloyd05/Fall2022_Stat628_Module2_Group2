...
ui <- 
  navbarPage("Group 2 Shiny", collapsible = TRUE, inverse = TRUE, theme = shinytheme("spacelab"),
             ...
             tabPanel("Model Calculator",
                      ...
                      sidebarLayout(sidebarPanel(numericInput("Weight", "Weight (in kg):", 0, min = 0, step = .1), numericInput("Abdomen", "Abdomen Circumfrence (in cm):", 0, min = 0, step = .1)),
...
server <- function(input, output) { 
  ...
  output$calculated_bodyfat <- renderText(paste(c(-40.47 - .673*input$Weight + .91 * input$Abdomen),"%"))
